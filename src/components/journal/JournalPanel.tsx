                  ...
                  {sectionDraftItems.map((draftItem) => (
                    <JournalInlineItem
                      ...
                      item={{
                        id: draftItem.id,
                        case_id: caseId,
                        situation_id: selectedSituationId ?? null,
...
export function JournalPanel({
  ...
  useEffect(() => {
    ...
    async function loadJournalItems() {
      try {
        ...
        const response = await fetch(`/api/cases/${caseItem.id}/journal?situationId=${encodeURIComponent(selectedSituationId)}`, {
          cache: "no-store"
        });
  ...
  async function handleCreateDraftJournalItem(draftId: string, title: string) {
    ...
    if (!draftItem || !selectedSituationId) {
      setJournalError("Nejdřív vyber situaci.");
      return;
    }
